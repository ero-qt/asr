...
fn classes(manager: &SceneManager) -> Vec<Address> {
    with_process(&[(BASE, &image(manager))], |process: &Process| {
        let transform = Transform {
            address: Address::new(BASE + TRANSFORM),
        };
        transform.classes(process, manager).unwrap().collect()
    })
}
...
#[test]
fn classes_reach_the_managed_objects_through_the_cached_object() {
...
#[test]
fn classes_reach_the_managed_objects_through_the_slot() {
...
#[test]
fn find_class_takes_the_object_whose_class_carries_the_name() {
    let manager = manager((2023, 1, 22, 16744));
    with_process(&[(BASE, &image(&manager))], |process: &Process| {
        let transform = Transform {
            address: Address::new(BASE + TRANSFORM),
        ...
        assert_eq!(
            transform
                .find_class(process, &manager, "Mover", class_name_of)
                .unwrap(),
            Address::new(BASE + MOVER_OBJECT)
        );
        assert!(transform
            .find_class(process, &manager, "Camera", class_name_of)
            .is_err());
    });
}
