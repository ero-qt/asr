...
#[test]
fn the_first_inline_path_build_is_2021_3_0() {
    let build = builds::nearest((2021, 3, 0, 44232), PointerSize::Bit64).unwrap();
    assert_eq!(build.unity, (2021, 3, 0, 44232));
    assert_eq!(build.profile.path, PathShape::InlineNul);
    let before = builds::nearest((2020, 2, 0, 8671), PointerSize::Bit64).unwrap();
    assert_eq!(before.profile.path, PathShape::Pointer);
}
...
fn a_short_path_reads_inline_up_to_the_nul() {
    let manager = manager((2021, 3, 0, 44232), PointerSize::Bit64);
    let mut image = vec![0; 0x1000];
    let at = field(&manager);
